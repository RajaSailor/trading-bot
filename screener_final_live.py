"""
LIVE SCREENER - Using Correct dhanhq() API
5-Min Candle Breakout Strategy with correct initialization
File: screener_final_live.py
"""
...
print(f"""
{'='*90}
📊 LIVE SCREENER - FINAL VERSION (5 MIN BREAKOUT STRATEGY)
{'='*90}
[INFO] Configuration:
...
try:
    ...
    # Initialize dhanhq API with context
...
# Symbol to exchange/code mapping
symbol_config = {
    "NIFTY": {"exchange": "NSE", "code": "NIFTY"},
    "BANKNIFTY": {"exchange": "NSE", "code": "BANKNIFTY"},
    "CRUDEOIL": {"exchange": "MCX", "code": "CRUDEOIL"},
}
...
def fetch_quote(symbol):
    ...
    try:
        ...
        # Call get_quote
        response = dhan_api.get_quote(exchange, code)
        ...
        if response and response.get('status') == 'success':
            ...
            return {
                ...
                "high": data.get("high_price", data.get("high", 0)),
                "low": data.get("low_price", data.get("low", 0)),
                "open": data.get("open_price", data.get("open", 0)),
                "close": data.get("close_price", data.get("close", 0)),
            }
    except Exception as e:
        pass
