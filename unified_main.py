...
class UnifiedTradingSystem:
    ...
    def __init__(self) -> None:
        ...
        self.dhan_client = DhanAPIClient(access_token, token_refresh_callback=self.token_manager.token_generator)
    ...
    def _execute_approved_trades(self) -> None:
        for request in self.trade_control_handler.approved_for_execution():
            ...
            order_id = str(response.get("orderId") or response.get("order_id") or f"SIM-{request.trade_id}")
            ...
            state = self.trailing_stop_manager.initialize(
                ...
                entry_price=executed.entry,
                stop_loss=executed.stop_loss,
                targets=executed.targets,
            )
            state = self.trailing_stop_manager.on_entry_filled(state, executed.entry)
